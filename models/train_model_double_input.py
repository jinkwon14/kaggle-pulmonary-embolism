...
from tensorflow.keras import regularizers, optimizers, Input
from tensorflow.keras import Model
from tensorflow.keras.applications import DenseNet121

from data_generator_double_v10082020 import DataGenerator
import os
...
img_nums_dict = dict(zip(train_df['SOPInstanceUID'], train_df['img_no']))
...
# add image number input
image_no_input = Input(shape = (1, ), name = 'img_location')

headModel = Concatenate()([headModel, image_no_input])
...
model = Model(
    inputs=[baseModel.input, image_no_input],
    outputs=[rspe, lspe, cpe, acpe, cnpe, qamo, qaco, rv_gt1, rv_lt1]
    )
    # tensorflow.keras.utils.plot_model(model, "kaggle_pulmonary_embolism_v10032020.png", show_shapes=True)
...
load_checkpoint_path = 'training_10082020/cp.ckpt'
...
import datetime
logdir = "logs/scalars/double_input_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
...
save_checkpoint_path = "training_10082020/cp_10082020_double_input.ckpt"

# Create a callback that saves the model's weights
cp_callback = tensorflow.keras.callbacks.ModelCheckpoint(filepath=save_checkpoint_path,
                                                 save_weights_only=True,
                                                 verbose=1)
csv_logger_callback = CSVLogger("model_history_log_v10082020_double_input.csv", append=True)
